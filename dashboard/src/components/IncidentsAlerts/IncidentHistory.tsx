...
function IncidentHistory() {
  ...
  const getResponseTime = (event: EmergencyEvent) => {
    if (!event.firstResponseTime) return null
    return differenceInMinutes(event.firstResponseTime, event.detectedAt)
  }

  const getResolutionTime = (event: EmergencyEvent) => {
    if (!event.resolutionTime || !event.firstResponseTime) return null
    return differenceInMinutes(event.resolutionTime, event.firstResponseTime)
  }
                    ...
                    <TableRow key={event.id} hover>
                      <TableCell>
                        ...
                        <Typography variant="caption" color="text.secondary">
                          {format(event.detectedAt, 'HH:mm:ss')}
                        </Typography>
                      </TableCell>
                      <TableCell>
                        <Typography variant="body2">
